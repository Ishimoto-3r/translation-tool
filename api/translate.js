...
`;
  ...
  // プロンプト構築（共通化）
  const systemPrompt = buildRowsSystemPrompt(toLang, kind, contextPrompt);
...
// ------------------------------------------------------------
// プロンプト構築（word / sheet / verify 共通化）
// ------------------------------------------------------------
function buildRowsSystemPrompt(toLang, kind, contextPrompt) {
  // kind固有の追加ルール
  const kindSpecific = kind === "word"
    ? `
