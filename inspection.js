// inspection.js（全文）
console.log("[inspection] front version: v9");
...
async function extractPdfTextWithPdfjs(file) {
  if (!window.pdfjsLib) {
    throw new Error("pdf.js が読み込めません（CDNブロックの可能性）");
  }
  const buf = await file.arrayBuffer();
  const loadingTask = window.pdfjsLib.getDocument({ data: buf });
  const pdf = await loadingTask.promise;

  const maxPages = Math.min(pdf.numPages, 30); // 上限：30ページ（マニュアル用途）
  const texts = [];
...
async function runExtract() {
  ...
  try {
    ...
    const filename = encodeURIComponent(pdfFile.name || "manual.pdf");

    // 大きいPDFはボディ上限(413)回避のため、ブラウザでテキスト抽出してJSON送信に切り替える
    const useTextMode = (pdfFile.size || 0) > 4 * 1024 * 1024;

    async function postPdfBinary() {
      const ab = await pdfFile.arrayBuffer();
      return fetch("/api/inspection?op=extract", {
        method: "POST",
    ...
    async function postPdfText() {
      const pdfText = await extractPdfTextWithPdfjs(pdfFile);
      return fetch("/api/inspection?op=extract", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileName: decodeURIComponent(filename), pdfText }),
      });
    }

    let res = null;
    if (useTextMode) {
      res = await postPdfText();
    } else {
      res = await postPdfBinary();
      if (res.status === 413) {
        res = await postPdfText();
      }
    }
